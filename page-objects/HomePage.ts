...
export class Homepage extends BasePage {
  ...
  async getProductImg(productName: string): Promise<Locator> {
    return this.page.locator(`//div[contains(@class, 'card-block')]//a[contains(text(), '${productName}')]//ancestor::div[@class='card h-100']//img`);
  }
  ...
  async verifyCatergoryMenuContainsCorrectItems(items: string[]): Promise<void> {
    for (const item of items) {
      const categoryItem = this.categoriesMenu.locator(`text=${item}`);
