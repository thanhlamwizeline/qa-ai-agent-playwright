...
export class Homepage extends BasePage {
  ...
  async getProductImg(productName: string): Promise<Locator> {
    const selectedProduct = this.card_ProductItem.filter({ hasText: productName });
  ...
  async verifyCatergoryMenuContainsCorrectItems(items: string[]): Promise<void> {
    for (const item of items) {
      const categoryLink = this.categoriesMenu.getByRole('link', { name: item });
  ...
  async verifyProductIsDisplayed(productName: string): Promise<void> {
    const productCard = this.productList.locator('.card-title').filter({ hasText: productName });
