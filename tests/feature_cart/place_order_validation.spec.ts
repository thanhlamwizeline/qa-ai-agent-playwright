...
test.describe('Place Order Validation', () => {
  ...
  test('@QAAGENT-61 Verify that User can place an order', async ({ page }) => {
    // Step 1: Navigate to homepage (done in beforeEach)
    ...
    await poManager.getCartPage().verifyThankYouMessage();
  });
});